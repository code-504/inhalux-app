...
export function RelationProvider({ children }: Props) {
    ...
    const fetchShareData = useCallback(async () => {
      ...
      `
            ...
            let transformedShareData = data
                .filter((patient: any) => patient.user !== null)
                .map(
                    (share: any): ListMonitor => ({
                        id: share.user.id,
                        name:
                            share.user.name +
                            (share.user.last_name
                                ? " " + share.user.last_name
                                : ""),
                        avatar: share.user.avatar,
                        kindred: share.name_from_monitor
                            ? share.name_from_monitor
                            : "Relativo",
    ...
    useEffect(() => {
        fetchPacientsData();
    }, [pacientState.filterText, session]);

    useEffect(() => {
        fetchShareData();
    }, [shareState.filterText, session]);
...
export const useRelations = () => {
    ...
    if (context === undefined) {
        throw new Error(
            `useRelation must be used within a MyUserContextProvider.`
        );
    }
    return context;
};
