...
const TabTwoScreen = () => {
    ...
    const [buttonState, setButtonState] = useState<ButtonLocationState>(
        ButtonLocationState.Inactive
    );
    ...
    const requestForegroundPermission = async () => {
        ...
        if (status !== "granted") {
            ...
            if (!permissionChange) setPermissionChange(true);
    ...
    const getCurrentPosition = async () => {
        const { foregroundPermission, hasLocationEnabled } =
    ...
    const getNearbyPharmacies = async () => {
        ...
        if (!foregroundPermission || !hasLocationEnabled) return;
        ...
        useCallback(() => {
            const onBackPress = () => {
                if (bottomSheetState.secondSheetActive) {
                    ...
                    if (navigatorB.canGoBack()) navigatorB.goBack();
                }
            ...
            <Ripple onPress={() => handleStoreItemPress(item)}>
                <View style={stylesStoreItem.container}>
                    <View style={stylesStoreItem.infoView}>
                        <MontserratSemiText style={stylesStoreItem.title}>
                            {item.name}
                        </MontserratSemiText>
                ...
                <View style={styles.container}>
                    ...
                    >
                        <View style={styles.buttonView}>
                            <Button
                                style={styles.ubicationButton}
                                onPress={handleGetLocation}
                                alignSelf="center"
                                size="$6"
                                circular
                            >
                                {buttonState === ButtonLocationState.Loading ? (
                                    <Spinner />
                                ) : buttonState ===
                                  ButtonLocationState.Inactive ? (
                                    <LocationDisabledIcon />
                                ) : buttonState ===
                                  ButtonLocationState.Active ? (
                                    <LocationUnknowIcon />
                                ) : (
                                    buttonState ===
                                        ButtonLocationState.Current && (
                                        <LocationCurrentIcon />
                                    )
                                )}
                            </Button>
                            ...
                            <Button
                                style={[
                                    ...
                                    {
                                        opacity:
                                            bottomSheetState.storeButtonVisible
                                                ? 1
                                                : 0,
                                    },
                                ]}
                                onPress={getNearbyPharmacies}
                                size="$6"
                                borderRadius={1000}
                            >
                                <StoreIcon />
                        ...
                        {inhaler && (
                            <View style={stylesInhalerSheet.content}>
                                <View style={stylesInhalerSheet.titleView}>
                                    ...
                                    <MontserratText
                                        style={stylesInhalerSheet.address}
                                    >
                                        {inhaler.address}
                                    </MontserratText>
                                    ...
                                    <View
                                        style={
                                            stylesInhalerSheet.inahlerStatusInfo
                                        }
                                    >
                                        <BatteryIcon
                                            style={
                                                stylesInhalerSheet.inahlerStatusIcon
                                            }
                                        />
                                        <MontserratSemiText>
                                            {inhaler.battery}%
                                ...
                                <View style={stylesInhalerSheet.buttonsView}>
                                    ...
                                    >
                                        <View
                                            style={
                                                stylesInhalerSheet.buttonTextView
                                            }
                                        >
                                            <SoundIcon />
                                            <MontserratBoldText>
...
TaskManager.defineTask(TASK_NAME, async ({ data: { locations }, error }) => {
    ...
    if (locations) {
        const { error: supabaseError } = await supabase
            .from("inhaler_ubication")
            .update({
                latitude: locations[0].coords.latitude,
                longitude: locations[0].coords.longitude,
            })
            .eq("id", "78d0246e-9444-468b-8e2f-7d9c48b6bbfe");
